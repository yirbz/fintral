...
import { QuickBooksIcon, XeroIcon, OdooIcon } from "@/components/brand-icons"
...
import {
  getXeroAuthUrl, listXeroConnections, deleteXeroConnection, testXeroConnection, pushToXero,
} from "@/lib/api/xero"
import type { XeroConnection, XeroTestResult, XeroPushResult } from "@/lib/api/xero"
import { toast } from "sonner"

function openQbPopup(onConnected?: () => void) {
  getQuickBooksAuthUrl().then(({ url }) => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (e.data.status === "connected") { toast.success("QuickBooks conectado"); onConnected?.(); }
...
function openXeroPopup(onConnected?: () => void) {
  getXeroAuthUrl().then(({ url }) => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (e.data.status === "connected") { toast.success("Xero conectado"); onConnected?.(); }
      else { toast.error("Error al conectar", { description: e.data.detail || "Error desconocido" }); }
    };
    window.addEventListener("message", handler);
...
type ExportFormatId =
  | "csv" | "quickbooks" | "xero" | "odoo" | "json" | "excel" | "txt"
...
const FORMATS: FormatBrand[] = [
  ...
  { id: "txt", name: "TXT", description: "Formato legible", extension: ".txt", icon: <FileText className="size-4" />, bg: "bg-orange-100 text-orange-700" },
...
export function ExportsPage() {
  ...
  const [xeroSelectedId, setXeroSelectedId] = useState("")
  const [xeroPushing, setXeroPushing] = useState(false)
  const [xeroPushResult, setXeroPushResult] = useState<XeroPushResult | null>(null)
  const [xeroTestResult, setXeroTestResult] = useState<XeroTestResult | null>(null)
  ...
  const loadXeroConnections = useCallback(async () => {
    try {
      const list = await listXeroConnections()
      setXeroConnections(list)
      if (list.length > 0 && !xeroSelectedId) {
        setXeroSelectedId(list[0].id)
      }
    } catch { /* silent */ }
  }, [xeroSelectedId])

  useEffect(() => { loadXeroConnections() }, [loadXeroConnections])
  ...
  async function handleDownload() {
    ...
    try {
      const FORMAT_MAP: Record<string, string> = {
        ...
        txt: "txt", odoo: "odoo", json: "json", excel: "excel",
      }
      ...
      const ext = selectedFormat === "excel" ? ".xlsx" : selectedFormat === "json" ? ".json" : selectedFormat === "txt" ? ".txt" : ".csv"
                ...
                <Card className="border-cyan-200/50">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="flex items-center justify-center rounded bg-cyan-100 text-cyan-700 size-6">
                          <XeroIcon className="size-3.5" />
                        </div>
                        <CardTitle className="text-sm">Push directo a Xero</CardTitle>
                      </div>
                      {xeroPushResult && (
                        <Badge className={xeroPushResult.failed === 0 ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}>
                          {xeroPushResult.success} enviadas
                          {xeroPushResult.failed > 0 && ` · ${xeroPushResult.failed} fallidas`}
                        </Badge>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      Envía facturas seleccionadas como ACCPAY a Xero vía API
                    </p>
                  </CardHeader>
                  <CardContent className="pt-0 space-y-3">
                    {xeroConnections.length > 0 ? (
                      <>
                        <div className="flex items-center gap-2">
                          <div className="flex-1 text-xs text-muted-foreground truncate">
                            {xeroConnections[0].name}{xeroConnections[0].last_error ? " ⚠️" : ""}
                          </div>
                          <Button
                            variant="ghost" size="icon-sm" className="size-7 shrink-0"
                            onClick={async () => {
                              if (!xeroSelectedId) return
                              setXeroTestResult(null)
                              try {
                                const r = await testXeroConnection(xeroSelectedId)
                                setXeroTestResult(r)
                                if (r.ok) toast.success(`Conexión exitosa · ${r.company_name || "Xero"}`)
                                else toast.error("Error de conexión", { description: r.error })
                              } catch { setXeroTestResult({ ok: false, error: "Error de conexión" }); toast.error("No se pudo conectar con Xero") }
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              if (!xeroSelectedId) return
                              try {
                                await deleteXeroConnection(xeroSelectedId)
                                setXeroSelectedId("")
                                loadXeroConnections()
                        ...
                        {xeroTestResult && (
                          <div className={cn(
                            ...
                            {xeroTestResult.ok
                              ? `✓ Conectado · ${xeroTestResult.company_name || "Xero"}`
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={async () => {
                              if (!xeroSelectedId) return
                              if (selectedIds.size === 0) {
                                toast.error("Selecciona al menos una factura para enviar a Xero")
                                return
                              }
                              setXeroPushing(true)
                              ...
                              try {
                                ...
                                const r = await pushToXero(xeroSelectedId, ids)
                                setXeroPushResult(r)
                                if (r.failed === 0) toast.success(`${r.success} facturas enviadas a Xero`)
                                ...
                                setXeroPushResult({ total: 0, success: 0, failed: 1, results: [{ invoice_id: "", invoice_number: null, success: false, error: e.message }] })
                                toast.error("Error al enviar a Xero", { description: e.message })
                              } finally { setXeroPushing(false) }
                            }}
                          >
                            <XeroIcon className="size-3" />
                            {xeroPushing ? "Enviando..." : "Push a Xero"}
                          </Button>
                          <Button
                            variant="outline" size="sm" className="h-7 text-xs gap-1"
                            onClick={() => openXeroPopup(loadXeroConnections)}
                          >
                            <RefreshCw className="size-3" />
                            Reconectar
                          </Button>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <p className="text-xs text-muted-foreground">
                          Conecta tu cuenta de Xero para enviar facturas directamente
                        </p>
                        <Button
                    ...
                    {xeroPushResult && xeroPushResult.results.length > 0 && (
                      <div className="text-xs space-y-1 max-h-32 overflow-y-auto border rounded p-2">
                        {xeroPushResult.results.map((r, i) => (
                          <div key={i} className={cn("flex items-center gap-2", r.success ? "text-green-600" : "text-red-600")}>
                            ...
                            {r.xero_invoice_id && <span className="text-muted-foreground">→ ID Xero: {r.xero_invoice_id}</span>}
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={async () => {
                              if (!qbSelectedId) return
                              if (selectedIds.size === 0) {
                                toast.error("Selecciona al menos una factura para enviar a QuickBooks")
                                return
                              }
                              setQbPushing(true)
                          ...
                          <Button
                            variant="outline" size="sm" className="h-7 text-xs gap-1"
                            onClick={() => openQbPopup(loadQbConnections)}
                          >
                            <RefreshCw className="size-3" />
                            Reconectar
                          </Button>
