...
function getColLetter(idx: number) {
  return String.fromCharCode(65 + idx); // A, B, C, ...
}

function PreviewContent({ reportType, onClose }: { reportType: "ap" | "ar"; onClose: () => void }) {
            ...
            <table className="border-collapse w-full table-fixed select-none">
              ...
              <tbody className="bg-white dark:bg-slate-950 font-sans text-xs text-slate-800 dark:text-slate-200">
                ...
                <tr className="h-6">
                  ...
                  <td
                    colSpan={data.headers.length}
                    className="border border-slate-200 dark:border-slate-800 px-3 font-medium text-xs text-slate-600 dark:text-slate-200 align-middle bg-indigo-50/20"
                  >
                    {data.report_name}
                  </td>
                </tr>
                <tr className="h-6">
                  ...
                  <td
                    colSpan={data.headers.length}
                    className="border border-slate-200 dark:border-slate-800 px-3 text-xs text-slate-700 dark:text-slate-100 align-middle bg-indigo-50/20"
                  >
                ...
                <tr className="h-6">
                  ...
                  <td
                    colSpan={data.headers.length}
                    className="border border-slate-200 dark:border-slate-800 px-3 text-[11px] text-slate-500 dark:text-slate-300 italic align-middle bg-indigo-50/20"
                  >
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[95vw] lg:max-w-[85vw] xl:max-w-[75vw] max-h-[90vh] flex flex-col p-6 overflow-hidden">
        <PreviewContent key={String(isOpen)} reportType={reportType} onClose={onClose} />
      </DialogContent>
    </Dialog>
  );
}
