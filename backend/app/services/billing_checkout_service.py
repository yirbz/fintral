...
class BillingCheckoutService:
    ...
    async def process_complete_cart(
        ...
        # Addons (except ecf_blocks) require an active subscription if no plan change is present
        has_plan_change = len(plan_items) > 0
        has_postpay_addons = any(i.get("type") in ("entity_slot", "user_slot", "ai", "storage", "ocr") for i in items)
        if has_postpay_addons and not has_plan_change:
            if not current_sub or current_sub.status != "active":
                raise ValueError("Los complementos de capacidad y recursos solo pueden adquirirse con una suscripción activa de pago.")
